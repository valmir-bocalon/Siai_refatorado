...
type
  TFrmRecebBaixa = class(TForm)
    ...
    procedure XNEJurosExit(Sender: TObject);
...
begin
  ...
  DM_Tabelas.ZQCheque.SQL.Add('select * from cheque_rec limit 0,1');
  DM_Tabelas.ZQCheque.open;
...
{  DM_tabelas.ZQRecebimento.Close;
  ...
  DM_tabelas.ZQReceb_Baixa.SQL.Add('Select * from Receb_Baixa limit 0,1');
...
begin
  // 24/05/2012
//  if (XNEMarcado.Value > XNERecebido.Value) and (XNEDesconto.Value=0) Then
...
//     XNEResto.Value := XNEMarcado.Value - (XNERecebido.Value - XNEDesconto.Value)
     XNEResto.Value := ((XNEMarcado.Value+XNEJuros.Value)-XNEDesconto.Value) - (XNERecebido.Value )
   else
...
//          DM_Tabelas.ZQChequeidvinculo.Value := DM_Tabelas.ZQRecebimentoidrecebimento.Value;
    ...
    Begin
      ...
      DM_Tabelas.ZQReBxHi.FieldByName('data').Value := strtodate(JDEntrada.DateText); //JDBaixa.DateText;
...
//        ZQRecBaisq.Value := contador;
    ...
    ZQRecebimento.sql.add('Select * from recebimento where quadralote='+quotedstr(quadra)+' group by quadralote order by quadralote');
    ZQRecebimento.Open;
    ZQRecebimento.First;
    bar1.Position := 0;
    ...
    begin
      ...
      ZQEntrada.Close;
      ZQEntrada.SQL.Clear;
      ZQEntrada.SQL.Add('Select *, sum(valor) as entrada from recebimento where tip<>''J'' and quadralote='+quotedstr(ZQRecebimento.FieldByName('quadralote').Value)+' and documento like  ''%-E-%''  order by ordem');
      ZQEntrada.open;

      ZQParcela.Close;
      ZQParcela.SQL.Clear;
      ZQParcela.SQL.Add('Select *, sum(valor) as parcela from recebimento where tip<>''J'' and quadralote='+quotedstr(ZQRecebimento.FieldByName('quadralote').Value)+' and documento like  ''%-P-%''  order by ordem');
      ZQParcela.open;
      if (ZQEntrada.FieldByName('entrada').Value>0) or (ZQParcela.FieldByName('parcela').Value>0) then
      begin
        ZQVenda.SQL.Clear;
        ZQVenda.SQL.Add('Select * from venda where idvenda='+quotedstr(ZQRecebimento.FieldByName('venda_idvenda').Text));
        ZQVenda.open;
...
//        ZQRecBaisq.Value := contador;
    ...
    begin
      ...
      ZQEntrada.Close;
      ZQEntrada.SQL.Clear;
      ZQEntrada.SQL.Add('Select *, sum(valor) as entrada from recebimento where quadralote='+quotedstr(ZQRecebimento.FieldByName('quadralote').Value)+' and documento like  ''%-E-%''  order by ordem');
      ZQEntrada.open;

      ZQParcela.Close;
      ZQParcela.SQL.Clear;
      ZQParcela.SQL.Add('Select *, sum(valor) as parcela from recebimento where quadralote='+quotedstr(ZQRecebimento.FieldByName('quadralote').Value)+' and documento like  ''%-P-%''  order by ordem');
      ZQParcela.open;
      if (ZQEntrada.FieldByName('entrada').Value>0) or (ZQParcela.FieldByName('parcela').Value>0) then
      begin
        ZQVenda.SQL.Clear;
...
begin
  ...
  Begin
    ...
    Begin
      showmessage('O Descri��o cont?il n? pode ficar em branco....');
      Econtabil.SetFocus;
      exit;
    end;
...
Begin
  ...
  begin
    ...
    begin
       ...
       begin
         ZQRecBai.SQL.Add('select * from recebimento where (quadralote='+quotedstr(Elote.text)+') and (recpag='+quotedstr(varrecpag)+') and ((saldo>0) and  (not saldo is null)) and (cliente ='+Ecodnome.Text+')');
         vazio:='F';
       end
       else
       begin
         showmessage('Dados N? Encontrados!');
         vazio:='T';
         exit;
       end;  
...
procedure TFrmRecebBaixa.XNEJurosExit(Sender: TObject);
begin
   if XNEMarcado.Value >= XNERecebido.Value Then
//     XNEResto.Value := XNEMarcado.Value - (XNERecebido.Value - XNEDesconto.Value)
     XNEResto.Value := ((XNEMarcado.Value+XNEJuros.Value)-XNEDesconto.Value) - (XNERecebido.Value )
    ...
    Begin
      ...
      Begin
        ...
        begin
          ...
          if pfim.Text='  /  /    ' then
          begin
            DM_tabelas.ZQRecebimento.SQL.Add('Select * from recebimento where recpag=''R'' and idloteamento='+quotedstr(copy(EDLoteamento.Text,1,posi))+' and adversa='+quotedstr(EcodAdversa.Text)+' and saldo>0');
            DM_tabelas.ZQRecebimento.open;
          end
          else
          begin
            DM_tabelas.ZQRecebimento.SQL.Add('Select * from recebimento where recpag=''R'' and idloteamento='+quotedstr(copy(EDLoteamento.Text,1,posi))+' and adversa='+quotedstr(EcodAdversa.Text)+' and Dt_Vencimento>= :dt1 and Dt_Vencimento<= :dt2 and saldo>0');
        ...
        begin
          ...
          if pfim.Text='  /  /    ' then
          begin
            DM_tabelas.ZQRecebimento.SQL.Add('Select * from recebimento where recpag=''R'' and adversa='+quotedstr(EcodAdversa.Text)+' and saldo>0');
            DM_tabelas.ZQRecebimento.open;
          end
          else
          begin
            DM_tabelas.ZQRecebimento.SQL.Add('Select * from recebimento where recpag=''R'' and adversa='+quotedstr(EcodAdversa.Text)+' and Dt_Vencimento>= :dt1 and Dt_Vencimento<= :dt2 and saldo>0');
            DM_tabelas.ZQRecebimento.ParamByName('dt1').Value:=strtodate(pini.text);
            DM_tabelas.ZQRecebimento.ParamByName('dt2').Value:=strtodate(pfim.text);
            DM_tabelas.ZQRecebimento.open;
          end;
  ...
  begin
    ...
    begin
      ...
      begin
        DM_tabelas.ZQRecebimento.Close;
        DM_tabelas.ZQRecebimento.SQL.Clear;
        DM_tabelas.ZQRecebimento.SQL.Add('Select * from recebimento where idrecebimento ='+quotedstr(ZQCheque.FieldByName('idrecebimento').text)+' and recpag=''R'' and saldo>0');
        DM_tabelas.ZQRecebimento.open;
        if DM_tabelas.ZQRecebimento.RecordCount>0 then
        begin
    ...
    begin
      ZQCheque.close;
      showmessage('N?ero de Cheque n? Encontrado.');
      dxbfechar.setfocus;
    end;
  end;
  showmessage('Cheque n? Encontrado nas Parcelas em Aberto.');
  DM_tabelas.ZQRecebimento.Close;
  DM_tabelas.ZQRecebimento.SQL.Clear;
...
begin
   if XNEMarcado.Value > XNERecebido.Value Then
//     XNEResto.Value := XNEMarcado.Value - (XNERecebido.Value - XNEDesconto.Value)
     XNEResto.Value := (XNERecebido.Value )-((XNEMarcado.Value+XNEJuros.Value)-XNEDesconto.Value);
//   else  if XNEMarcado.Value > XNERecebido.Value Then
//      XNEDesconto.Value :=  XNERecebido.Value - XNEMarcado.Value;
   if (XNEMarcado.Value > XNERecebido.Value) and (XNEDesconto.Value=0) Then
       XNEResto.Value :=xdesc;
    xdesc:=0;
end;
...
begin
 ...
 begin
  ...
  begin
    ...
    DM_tabelas.ZQRecebimento.Close;
    DM_tabelas.ZQRecebimento.SQL.Clear;
    DM_tabelas.ZQRecebimento.SQL.Add('Select * from recebimento where recpag=''R'' and  idloteamento='+quotedstr(copy(EDLoteamento.Text,1,posi))+' and (dt_vencimento between :dt1 and :dt2) and saldo>0');
  ...
  begin
    ...
    DM_tabelas.ZQRecebimento.Close;
    DM_tabelas.ZQRecebimento.SQL.Clear;
    DM_tabelas.ZQRecebimento.SQL.Add('Select * from recebimento where recpag=''R'' and (dt_vencimento between :dt1 and :dt2) and saldo>0');
    DM_tabelas.ZQRecebimento.ParamByName('dt1').AsDate:=strtodate(PIni.text);
    DM_tabelas.ZQRecebimento.ParamByName('dt2').AsDate:=strtodate(PFim.text);
...
initialization
  ...
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempidrecbxhist', 'idrecbxhist', TLargeintField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  ...
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempvalor', 'valor', TFloatField, fkData, 0, 0, False, '', '###,###,##0.00', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempdescon', 'descon', TFloatField, fkData, 0, 0, False, '', '###,###,##0.00', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempjuros', 'juros', TFloatField, fkData, 0, 0, False, '', '###,###,##0.00', '', '', 0, '', '', '', '', False);
  ...
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempdata', 'data', TDateField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempidrecebimento', 'idrecebimento', TLargeintField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempdocumento', 'documento', TWideStringField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempcliente', 'cliente', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempusuario', 'usuario', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempDt_Entrada', 'Dt_Entrada', TDateField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempDt_Vencimento', 'Dt_Vencimento', TDateField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempValor_1', 'Valor_1', TFloatField, fkData, 0, 0, False, '', '###,###,##0.00', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempObserv', 'Observ', TWideMemoField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempVrDoc', 'VrDoc', TFloatField, fkData, 0, 0, False, '', '###,###,##0.00', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempordem', 'ordem', TWideStringField, fkData, 14, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempTipDoc', 'TipDoc', TWideStringField, fkData, 2, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempsaldo', 'saldo', TFloatField, fkData, 0, 0, False, '', '###,###,##0.00', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempmarcar', 'marcar', TWideStringField, fkData, 1, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempRefBaixa', 'RefBaixa', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTemprefvinda', 'refvinda', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempcontabil', 'contabil', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempempresa', 'empresa', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempcustodaparcela', 'custodaparcela', TFloatField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTemporigem', 'origem', TWideStringField, fkData, 10, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempadversa', 'adversa', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTemprecpag', 'recpag', TWideStringField, fkData, 1, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempnumordem', 'numordem', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempidreceb_baixa', 'idreceb_baixa', TLargeintField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempDt_rec', 'Dt_rec', TDateField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempVr_rec', 'Vr_rec', TFloatField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempTipDoc_1', 'TipDoc_1', TWideStringField, fkData, 2, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempDocum', 'Docum', TWideStringField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempJuros_Vr', 'Juros_Vr', TFloatField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempDesc_Vr', 'Desc_Vr', TFloatField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempRefBaixa_1', 'RefBaixa_1', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempobsebx', 'obsebx', TWideMemoField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempCodcontabil', 'Codcontabil', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempdataref', 'dataref', TDateField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempvencimento', 'vencimento', TDateField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempidloteamento', 'idloteamento', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempvenda_idvenda', 'venda_idvenda', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempquadralote', 'quadralote', TWideStringField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempSubstituicao', 'Substituicao', TWideStringField, fkData, 1, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrmRecebBaixa, 'ZQRecebBxTemp', 'ZQRecebBxTempsubstituicao_1', 'substituicao_1', TWideStringField, fkData, 1, 0, False, '', '', '', '', 0, '', '', '', '', False);
