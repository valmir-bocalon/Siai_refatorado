...
uses
  ...
  RLHTMLFilter, RLPDFFilter, RLXLSFilter, DBClient;

type
  TFrm_RelVenda03 = class(TForm)
    RLReport1: TRLReport;
    RLBand1: TRLBand;
...
var
  Frm_RelVenda03: TFrm_RelVenda03;
...
{$R *.dfm}
procedure TFrm_RelVenda03.RLBand2BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
//  RLReport1.NewPage;
end;

procedure TFrm_RelVenda03.RLBand7BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
...
//  begin
  ...
  if (Frm_RelVenda.ZQBxEntrada.FieldByName('data').AsDateTime=0) and (Frm_RelVenda.ZQEntrada.FieldByName('Dt_Vencimento').AsDateTime>date) then
       RLBand7.Color  := clWhite;
  if (Frm_RelVenda.ZQBxEntrada.FieldByName('data').AsDateTime=0) and (Frm_RelVenda.ZQEntrada.FieldByName('Dt_Vencimento').AsDateTime<date) then
     RLBand7.Color  := $00ECECFF; // vermelho
...
procedure TFrm_RelVenda03.RLReport1AfterPrint(Sender: TObject);
begin
  ZQincorp_loteame.close;
  PQincorp_loteame.close;
end;

procedure TFrm_RelVenda03.RLBand9BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
...
procedure TFrm_RelVenda03.RLBand16BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
...
{  abp:=abp+truncar(Frm_RelVenda.ZQParcelaValor.value,4);
  ...
  if (Frm_RelVenda.ZQBxParcela.FieldByName('data').AsDateTime=0) and (Frm_RelVenda.ZQParcela.FieldByName('Dt_Vencimento').AsDateTime>date) then
     RLBand16.Color  := clWhite;
 if (Frm_RelVenda.ZQBxParcela.FieldByName('data').AsDateTime=0) and (Frm_RelVenda.ZQParcela.FieldByName('Dt_Vencimento').AsDateTime<date) then     
     RLBand16.Color  := $00ECECFF; //Vermelho
  if (Frm_RelVenda.ZQParcela.FieldByName('Dt_Vencimento').AsDateTime>=Frm_RelVenda.ZQBxParcela.FieldByName('data').AsDateTime) and  ( Frm_RelVenda.ZQBxParcela.FieldByName('data').AsDateTime>0) then
     RLBand16.Color  := $00ECFFEC; // Verde
  if Frm_RelVenda.ZQParcela.FieldByName('Dt_Vencimento').AsDateTime<Frm_RelVenda.ZQBxParcela.FieldByName('data').AsDateTime then
     RLBand16.Color  := $00DFFFFF; //Amarelo


end;

procedure TFrm_RelVenda03.RLBand10BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
...
procedure TFrm_RelVenda03.RLBand3AfterPrint(Sender: TObject);
begin
...
procedure TFrm_RelVenda03.RLSubDetail4BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
  nparc.Caption:=inttostr(Frm_RelVenda.ZQParcela.recordcount);
end;

procedure TFrm_RelVenda03.ZQincorp_loteameAfterPost(DataSet: TDataSet);
begin
  ZQincorp_loteame.ApplyUpdates;
end;

procedure TFrm_RelVenda03.PQincorp_loteameAfterPost(DataSet: TDataSet);
begin
  PQincorp_loteame.ApplyUpdates;
end;

procedure TFrm_RelVenda03.RLBand17BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
...
procedure TFrm_RelVenda03.AfterConstruction;
begin
...
initialization
  RegisterRuntimeDataSet(TFrm_RelVenda03, 'ZQincorp_loteame', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteameincorporador_idincorporador', 'incorporador_idincorporador', TLargeintField, fkData, 0, 0, True, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteameloteamento_idloteamento', 'loteamento_idloteamento', TLargeintField, fkData, 0, 0, True, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteamepercent_parte', 'percent_parte', TFloatField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteamecodcontabancaria', 'codcontabancaria', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteamenconta', 'nconta', TWideStringField, fkLookup, 50, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_conta', True);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteameconta_v', 'conta_v', TWideStringField, fkLookup, 3, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_conta_v', True);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteameagencia', 'agencia', TWideStringField, fkLookup, 6, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_agencia', True);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteameagencia_v', 'agencia_v', TWideStringField, fkLookup, 3, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_agencia_v', True);
  RegisterRuntimeField(TFrm_RelVenda03, 'ZQincorp_loteame', 'ZQincorp_loteamen_banco', 'n_banco', TWideStringField, fkLookup, 3, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_banco', True);
  RegisterRuntimeDataSet(TFrm_RelVenda03, 'PQincorp_loteame', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteameincorporador_idincorporador', 'incorporador_idincorporador', TLargeintField, fkData, 0, 0, True, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteameloteamento_idloteamento', 'loteamento_idloteamento', TLargeintField, fkData, 0, 0, True, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteamepercent_parte', 'percent_parte', TFloatField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteamecodcontabancaria', 'codcontabancaria', TIntegerField, fkData, 0, 0, False, '', '', '', '', 0, '', '', '', '', False);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteamenconta', 'nconta', TWideStringField, fkLookup, 50, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_conta', True);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteameconta_v', 'conta_v', TWideStringField, fkLookup, 3, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_conta_v', True);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteameagencia', 'agencia', TWideStringField, fkLookup, 6, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_agencia', True);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteameagencia_v', 'agencia_v', TWideStringField, fkLookup, 3, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_agencia_v', True);
  RegisterRuntimeField(TFrm_RelVenda03, 'PQincorp_loteame', 'PQincorp_loteamen_banco', 'n_banco', TWideStringField, fkLookup, 3, 0, False, '', '', '', '', 0, 'codcontabancaria', 'DM_Tabelas.ZQContaBancaria', 'idconta_bancaria', 'n_banco', True);
